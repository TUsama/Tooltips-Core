...
public class VanillaTooltipsHandler {
    ...
    public void OnScreenPre(ScreenEvent.Render.Pre event){
        ...
        if (!TooltipsRecorder.addToFixed.isEmpty()) {
            Iterator<TooltipsRecorder.TooltipsRecord> iterator = TooltipsRecorder.addToFixed.iterator();
            while (iterator.hasNext()){
                TooltipsRecorder.TooltipsRecord next = iterator.next();

                TooltipsWidget fixed = new TooltipsWidget(event.getMouseX(), event.getMouseY(), 5, 5, next.provide(), next.itemStack(), event.getScreen());

                ((ScreenInvoker) screen).tc$addRenderableWidget(fixed);
